...
#import "SKPixelBufferDisplayView.h"
...
NSString *const SKFUYUVToRGBAFragmentShaderString = SHADER_STRING
(
...
NSString *const SKFURGBAFragmentShaderString = SHADER_STRING
(
...
NSString *const SKFUVertexShaderString = SHADER_STRING
(
...
NSString *const SKFUPointsFrgShaderString = SHADER_STRING
(
...
NSString *const SKFUPointsVtxShaderString = SHADER_STRING
(
 attribute vec4 position;
...
@interface SKPixelBufferDisplayView()
...
@implementation SKPixelBufferDisplayView {
    GLuint rgbaProgram;
...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame] ) {
        ...
        self.contextQueue = dispatch_queue_create("com.SKRender.contextQueue", DISPATCH_QUEUE_SERIAL);
...
- (BOOL)loadShadersRGBA{
    ...
    if (![self compileShader:&vertShader type:GL_VERTEX_SHADER string:SKFUVertexShaderString]) {
        NSLog(@"Failed to compile vertex shader");
        return NO;
    }
    
    // Create and compile fragment shader.
    if (![self compileShader:&fragShader type:GL_FRAGMENT_SHADER string:SKFURGBAFragmentShaderString]) {
...
- (BOOL)loadShadersYUV {
    ...
    // Create and compile fragment shader.
    if (![self compileShader:&fragShader type:GL_FRAGMENT_SHADER string:SKFUYUVToRGBAFragmentShaderString]) {
        NSLog(@"Failed to compile fragment shader");
        return NO;
    }
...
- (BOOL)loadPointsShaders {
    ...
    // Create and compile the vertex shader.
    if (![self compileShader:&vertShader type:GL_VERTEX_SHADER string:SKFUPointsVtxShaderString]) {
        NSLog(@"Failed to compile vertex shader");
        return NO;
    }
    
    // Create and compile fragment shader.
    if (![self compileShader:&fragShader type:GL_FRAGMENT_SHADER string:SKFUPointsFrgShaderString]) {
        NSLog(@"Failed to compile fragment shader");
        return NO;
    }
